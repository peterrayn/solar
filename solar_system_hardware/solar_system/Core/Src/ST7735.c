...
//
    ...
    HAL_GPIO_WritePin(RST_PORT, RST_PIN, GPIO_PIN_RESET);
    HAL_Delay(50);   // 【修改】从 5 改成 50，确保屏幕完全断电复位
    HAL_GPIO_WritePin(RST_PORT, RST_PIN, GPIO_PIN_SET);
    HAL_Delay(150);  // 【修改】从 120 改成 150，给足退出休眠的准备时间
}
...
//
    ...
    HAL_Delay(50);     // 【新增】等待单片机和屏幕电源彻底稳定
    ST7735_Unselect(); // 【修改】在复位前，确保 CS 为高电平
    ST7735_Reset();    // 执行复位
    ST7735_Select();   // 【修改】复位完成后，再拉低 CS 开始通信
